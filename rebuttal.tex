...
\usepackage{multirow}
\usepackage{diagbox}
% \usepackage{tabularx} % 提供更灵活的表格功能
\usepackage{array}
...
\newcommand{\no}{\text{\ding{55}}}
\newcommand{\blank}{—}
\usepackage{xspace}  % 为自定义指令添加自适应空白
...
\renewcommand{\thereviewer}{R\#\arabic{reviewer}}

\newcounter{comment}[reviewer]
\setcounter{comment}{0}
\renewcommand{\thecomment}{\arabic{comment} (\thereviewer{})} % 这里使用\thereviewer{}可以使用前面定义的形式
% \crefname{comment}{Comment}{Comments} % 为计数器指定\cref的单复数显式名称
\crefformat{comment}{Comment~#2#1#3} % 精细控制定义单引用形式
\crefrangeformat{comment}{Comments~#3#1#4-#5#2#6}  % 精细控制定义多引用形式
...
\begin{reply}
  ...
  This book is a treatise on the theory of ethics, very popular during the Renaissance~\cref{comment:1}.
...
\begin{reply}
  ...
  The generated Lorem Ipsum is therefore always free from repetition, injected humour, or non-characteristic words etc~\cref{comment:1,comment:2,comment:3,comment:4}.
\end{reply}
\begin{change}
...
\bibliography{mybib} % 你的文献库 mybib.bib
